        ...
        def leaf_callback_iterate_warc(data):
            with data() as stream_in:
                for record in ArchiveIterator(
                    stream_in, no_record_parse=False, arc2warc=True, verify_http=False
                ):
                    logger.debug(record.http_headers)

        for id, data, metadata in self._iterate_stream(
            storage_stream, leaf_callback=leaf_callback_iterate_warc
        ):
            pass

    def _iterate_stream(
        self, storage_stream: StorageStream, leaf_callback: Callable
    ) -> StorageStream:
        logger.debug(f"→ {self.prefix}{storage_stream}")
        for id, data, metadata in storage_stream:
            logger.debug(
                f"{self.prefix}{id}, {self._iterate_stream(data, leaf_callback=leaf_callback)}, {metadata}"
            )
            if isinstance(data, Callable):
                ...
                yield (
                    id,
                    self._iterate_stream(data, leaf_callback=leaf_callback),
                    metadata,
                )
